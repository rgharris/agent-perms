...
func TestClassifyPulumi(t *testing.T) {
	tests := []struct {
		...
		{name: "pulumi env run no inner", args: []string{"pulumi", "env", "run", "myorg/myenv"}, want: types.TierReadSensitiveRemote},
		{name: "pulumi env run kubectl get", args: []string{"pulumi", "env", "run", "myorg/myenv", "--", "kubectl", "get", "pods"}, want: types.TierReadSensitiveRemote},
		{name: "pulumi env run kubectl apply", args: []string{"pulumi", "env", "run", "myorg/myenv", "--", "kubectl", "apply", "-f", "x.yaml"}, want: types.TierWriteRemote},
		{name: "pulumi env run kubectl delete", args: []string{"pulumi", "env", "run", "myorg/myenv", "--", "kubectl", "delete", "pod", "x"}, want: types.TierAdminRemote},
		{name: "pulumi env run unknown cmd", args: []string{"pulumi", "env", "run", "myorg/myenv", "--", "bash"}, want: types.TierUnknown, unknown: true},
